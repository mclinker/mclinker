//===- lib/MC/ELFLDWriter.cpp - ELF File Writer -------------------===//
//
...
bool ELFLDWriter::isFixupKindPCRel(const MCAssembler &Asm, unsigned Kind) {
...
bool ELFLDWriter::RelocNeedsGOT(MCSymbolRefExpr::VariantKind Variant) {
  switch (Variant) {
  default:
    return false;
...
ELFLDWriter::~ELFLDWriter()
{}

// Emit the ELF header.
void ELFLDWriter::WriteHeader(uint64_t SectionDataSize,
...
void ELFLDWriter::WriteSymbolEntry(MCDataFragment *SymtabF,
...
uint64_t ELFLDWriter::SymbolValue(MCSymbolData &Data,
...
void ELFLDWriter::ExecutePostLayoutBinding(MCAssembler &Asm,
                                               const MCAsmLayout &Layout) {
...
void ELFLDWriter::WriteSymbol(MCDataFragment *SymtabF,
                                  MCDataFragment *ShndxF,
                                  ELFSymbolData &MSD,
                                  const MCAsmLayout &Layout) {
...
void ELFLDWriter::WriteSymbolTable(MCDataFragment *SymtabF,
                                       MCDataFragment *ShndxF,
...
const MCSymbol *ELFLDWriter::SymbolToReloc(const MCAssembler &Asm,
...
void ELFLDWriter::RecordRelocation(const MCAssembler &Asm,
                                       const MCAsmLayout &Layout,
...
uint64_t
ELFLDWriter::getSymbolIndexInSymbolTable(const MCAssembler &Asm,
                                             const MCSymbol *S) {
  MCSymbolData &SD = Asm.getSymbolData(*S);
  return SD.getIndex();
}

bool ELFLDWriter::isInSymtab(const MCAssembler &Asm,
...
bool ELFLDWriter::isLocal(const MCSymbolData &Data, bool isSignature,
                              bool isUsedInReloc) {
  if (Data.isExternal())
    return false;
...
void ELFLDWriter::ComputeIndexMap(MCAssembler &Asm,
...
void ELFLDWriter::ComputeSymbolTable(MCAssembler &Asm,
                                      const SectionIndexMapTy &SectionIndexMap,
...
void ELFLDWriter::CreateRelocationSections(MCAssembler &Asm,
                                               MCAsmLayout &Layout,
                                               RelMapTy &RelMap) {
...
void ELFLDWriter::WriteRelocations(MCAssembler &Asm, MCAsmLayout &Layout,
                                       const RelMapTy &RelMap) {
  for (MCAssembler::const_iterator it = Asm.begin(),
         ie = Asm.end(); it != ie; ++it) {
    const MCSectionData &SD = *it;
...
void ELFLDWriter::WriteSecHdrEntry(uint32_t Name, uint32_t Type,
...
void ELFLDWriter::WriteRelocationsFragment(const MCAssembler &Asm,
                                               MCDataFragment *F,
...
void ELFLDWriter::CreateIndexedSections(MCAssembler &Asm,
                                            MCAsmLayout &Layout,
                                            GroupMapTy &GroupMap,
                                            RevGroupMapTy &RevGroupMap,
                                            SectionIndexMapTy &SectionIndexMap,
...
void ELFLDWriter::WriteSection(MCAssembler &Asm,
                                   const SectionIndexMapTy &SectionIndexMap,
...
bool ELFLDWriter::IsELFMetaDataSection(const MCSectionData &SD) {
  return SD.getOrdinal() == ~UINT32_C(0) &&
    !SD.getSection().isVirtualSection();
}

uint64_t ELFLDWriter::DataSectionSize(const MCSectionData &SD) {
  uint64_t Ret = 0;
...
uint64_t ELFLDWriter::GetSectionFileSize(const MCAsmLayout &Layout,
                                             const MCSectionData &SD) {
...
uint64_t ELFLDWriter::GetSectionAddressSize(const MCAsmLayout &Layout,
                                                const MCSectionData &SD) {
  if (IsELFMetaDataSection(SD))
    return DataSectionSize(SD);
...
void ELFLDWriter::WriteDataSectionData(MCAssembler &Asm,
                                           const MCAsmLayout &Layout,
                                           const MCSectionELF &Section) {
...
void ELFLDWriter::WriteSectionHeader(MCAssembler &Asm,
...
void ELFLDWriter::ComputeSectionOrder(MCAssembler &Asm,
                                  std::vector<const MCSectionELF*> &Sections) {
  for (MCAssembler::iterator it = Asm.begin(),
         ie = Asm.end(); it != ie; ++it) {
    const MCSectionELF &Section =
...
bool
ELFLDWriter::IsSymbolRefDifferenceFullyResolvedImpl(const MCAssembler &Asm,
...
MCObjectWriter *llvm::createELFLDWriter(MCELFObjectTargetWriter *MOTW,
                                            raw_ostream &OS,
                                            bool IsLittleEndian) {
  switch (MOTW->getEMachine()) {
    case ELF::EM_386:
    case ELF::EM_X86_64:
      return new X86ELFLDWriter(MOTW, OS, IsLittleEndian); break;
    case ELF::EM_ARM:
      return new ARMELFLDWriter(MOTW, OS, IsLittleEndian); break;
    case ELF::EM_MBLAZE:
      return new MBlazeELFLDWriter(MOTW, OS, IsLittleEndian); break;
    default: llvm_unreachable("Unsupported architecture"); break;
  }
}


/// START OF SUBCLASSES for ELFLDWriter
//===- ARMELFLDWriter -------------------------------------------===//

ARMELFLDWriter::ARMELFLDWriter(MCELFObjectTargetWriter *MOTW,
                                       raw_ostream &_OS,
                                       bool IsLittleEndian)
  : ELFLDWriter(MOTW, _OS, IsLittleEndian)
{}

ARMELFLDWriter::~ARMELFLDWriter()
{}

// FIXME: get the real EABI Version from the Triple.
void ARMELFLDWriter::WriteEFlags() {
...
const MCSymbol *ARMELFLDWriter::ExplicitRelSym(const MCAssembler &Asm,
                                                   const MCValue &Target,
                                                   const MCFragment &F,
                                                   const MCFixup &Fixup,
                                                   bool IsPCRel) const {
...
// offset
unsigned ARMELFLDWriter::GetRelocType(const MCValue &Target,
                                          const MCFixup &Fixup,
                                          bool IsPCRel,
...
unsigned ARMELFLDWriter::GetRelocTypeInner(const MCValue &Target,
                                               const MCFixup &Fixup,
                                               bool IsPCRel) const  {
  MCSymbolRefExpr::VariantKind Modifier = Target.isAbsolute() ?
    MCSymbolRefExpr::VK_None : Target.getSymA()->getKind();
...
MBlazeELFLDWriter::MBlazeELFLDWriter(MCELFObjectTargetWriter *MOTW,
                                             raw_ostream &_OS,
                                             bool IsLittleEndian)
  : ELFLDWriter(MOTW, _OS, IsLittleEndian) {
}

MBlazeELFLDWriter::~MBlazeELFLDWriter() {
}

unsigned MBlazeELFLDWriter::GetRelocType(const MCValue &Target,
                                             const MCFixup &Fixup,
                                             bool IsPCRel,
                                             bool IsRelocWithSymbol,
                                             int64_t Addend) {
...
X86ELFLDWriter::X86ELFLDWriter(MCELFObjectTargetWriter *MOTW,
                                       raw_ostream &_OS,
                                       bool IsLittleEndian)
  : ELFLDWriter(MOTW, _OS, IsLittleEndian)
{}

X86ELFLDWriter::~X86ELFLDWriter()
{}

unsigned X86ELFLDWriter::GetRelocType(const MCValue &Target,
                                          const MCFixup &Fixup,
                                          bool IsPCRel,
                                          bool IsRelocWithSymbol,
                                          int64_t Addend) {
