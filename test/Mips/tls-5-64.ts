...
# RUN: %MCLinker -mtriple=mips64el-linux-gnu -e T0 -o %t.exe %t-exe.o %t.so

# RUN: llvm-readobj -r -dt %t.exe | FileCheck %s

# CHECK:      Relocations [
# CHECK-NEXT:   Section (5) .rel.dyn {
# CHECK-NEXT:     0x{{[0-9A-F]+}} R_MIPS_TLS_DTPMOD64/R_MIPS_NONE/R_MIPS_NONE T1 0x0
# CHECK-NEXT:     0x{{[0-9A-F]+}} R_MIPS_TLS_DTPREL64/R_MIPS_NONE/R_MIPS_NONE T1 0x0
# CHECK-NEXT:   }
# CHECK-NEXT: ]
