...
; RUN: -filetype=exe %t.o -o %t.so --color=never 2> %t.txt || exit 0
; RUN: test 0 -eq `cat %t.txt | grep -c '\[0;1;31m'`
...
entry:
  ...
  %2 = call i8* @__undefined_function_symbol(i32 %1)
  ret i8* %add.ptr
}

declare i8* @__undefined_function_symbol(i32)
