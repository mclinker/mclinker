; Check the interaction with -unique-section-names=0.
;
; LLVM adds an option -unique-section-names so that the compiler can generate
; same section name to reduce the memory overhead and intermediate object file
; size.  However, this means that we can't sort the .ARM.exidx fragments by
; their names.  The correct implementation should associate .text and
; .ARM.exidx sections via the linked section index.

; RUN: %LLC -mtriple=armv7-linux-gnueabi -filetype=obj \
; RUN:   -function-sections -unique-section-names=0 \
...
; CHECK: <test2+0x{{[0-9a-fA-F]+}}>: 0x1 [cantunwind]
